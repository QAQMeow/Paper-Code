# -*- coding: utf-8 -*-
"""
Created on Sat May 24 04:11:03 2025
...
CEA1 = pd.DataFrame({"NAME":mergeID["NAME"],"CNAME":MID['location_name'],'GP':MID['Income_Group'],'E': np.nanmean(A['a1'],axis=0)})
#pm.PLotGlobal(CEH,'Global Hazard',0,0.45,'Hazard value', cmaps.MPL_OrRd)
CEA2 = pd.DataFrame({"NAME":mergeID["NAME"],"CNAME":MID['location_name'],'GP':MID['Income_Group'],'E': np.nanmean(A['a2'],axis=0)})
#pm.PLotGlobal(CEE,'Global Exposure',0,0.65,'Exposure value', cmaps.MPL_YlGnBu)
CEA3 = pd.DataFrame({"NAME":mergeID["NAME"],"CNAME":MID['location_name'],'GP':MID['Income_Group'],'E': np.nanmean(A['a3'],axis=0)})
CEA4 = pd.DataFrame({"NAME":mergeID["NAME"],"CNAME":MID['location_name'],'GP':MID['Income_Group'],'E': np.nanmean(A['a4'],axis=0)})
...
PLotGlobal(axs[0,0],CEA1,'health expenditure(%)',0,30,'A', cmaps.  MPL_Blues[:100])
 
PLotGlobal(axs[0,1],CEA2,'emission intensity(t/$)',0,335,'B', cmaps.  MPL_Blues[:100])
 
PLotGlobal(axs[1,0],CEA3,'access to electricity(%)',59,100,'C', cmaps.  MPL_Blues[:100])
PLotGlobal(axs[1,1],CEA4,'Cook without clean fuels(%)',0.,100,'D', cmaps. MPL_Blues[:100])
 
fig.subplots_adjust(left=0.01,right=0.99,top=0.99,bottom=0.08,wspace=0.005,hspace=0.1)

plt.savefig('../res/Fig/fig_f/SI_fig7.png', dpi=300)