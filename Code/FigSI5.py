# -*- coding: utf-8 -*-
"""
Created on Fri May 23 11:14:11 2025
...
CEH1 = pd.DataFrame({"NAME":mergeID["NAME"],"CNAME":MID['location_name'],'GP':MID['Income_Group'],'E': np.nanmean(H['h1'],axis=0)})
#pm.PLotGlobal(CEH,'Global Hazard',0,0.45,'Hazard value', cmaps.MPL_OrRd)
CEH2 = pd.DataFrame({"NAME":mergeID["NAME"],"CNAME":MID['location_name'],'GP':MID['Income_Group'],'E': np.nanmean(H['h2'],axis=0)})
#pm.PLotGlobal(CEE,'Global Exposure',0,0.65,'Exposure value', cmaps.MPL_YlGnBu)
CEH3 = pd.DataFrame({"NAME":mergeID["NAME"],"CNAME":MID['location_name'],'GP':MID['Income_Group'],'E': np.nanmean(H['h3'],axis=0)})
...
fig,axs = plt.subplots(nrows=3, ncols=1,figsize=(width_inch, height_inch),dpi=300)
fig.patch.set_facecolor('#FFFFFF')
 
PLotGlobal(axs[0],CEH1,'duration(days)',0,14,'A', cmaps. sunshine_9lev)
 
PLotGlobal(axs[1],CEH2,'frequency',0,3,'B', cmaps. sunshine_9lev)
 
PLotGlobal(axs[2],CEH3,'intensity',0.,16,'C', cmaps. sunshine_9lev)

 
fig.subplots_adjust(left=0.01,right=0.99,top=0.99,bottom=0.08,wspace=0.005,hspace=0.1)

plt.savefig('../res/Fig/fig_f/SI_fig5.png', dpi=300)